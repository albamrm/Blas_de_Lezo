...
def colocador_aleatorio_barcos_jugador(tablero, Tm):
    ...
    for eslora in barcos:
        while True:
            ...
            y = random.randint(1, Tm+2 ) #el limite hay que verlo, por si Tm esta inluido
            ...
            if válido:
                ...
                break
def colocador_aleatorio_barcos_ordenador(Tm):
    coordenadas_barcos_ordenador = []
    barcos = [4, 3, 3, 2, 2, 2, 1, 1, 1, 1]
    direcciones = [(0, 1), (1, 0), (0, -1), (-1, 0)]
    for eslora in barcos:
        while True:
            ...
            for i in range(eslora):
                ...
                if not (1 <= nuevo_x <= (Tm*2+5) and 1 <= nuevo_y <= (Tm*2+5)):
                    válido = False
                    break
                coord = (nuevo_x, nuevo_y)
                if coord not in coordenadas_barcos_ordenador:
                    coordenadas_barcos_ordenador.append(coord)
                else:
                    válido = False
                    break
            if válido:
                break
    return coordenadas_barcos_ordenador
...
colocador_aleatorio_barcos_jugador(tablero_resultante, tamaño_tablero) #colocamos los barcos despues de crear el tablero
# Imprimimos el tablero resultante
tabla_info_pd = marcador(yo, enemigo)

coordenadas_barcos_ordenador = colocador_aleatorio_barcos_ordenador(tamaño_tablero)
print("Coordenadas de los barcos del ordenador:")
for coord in coordenadas_barcos_ordenador:
    print(coord)
